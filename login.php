<?php session_start(); if(isset($_SESSION['email']) && isset($_SESSION['user_id'])){  header('location:dashboard.php'); } ?><!DOCTYPE html>
...
<html lang="en">
	...
	<head>
		...
		<title>LancerShelf.com | Hire freelancers and artisans Online </title>
		<!-- start: META -->
...
.emmabold{
	...
	<body class="login">
		<!-- start: LOGIN -->
		<div class="row">
			<div class="main-login col-xs-12  col-sm-8 col-sm-offset-2 col-md-4 col-md-offset-4">
				<div class="logo margin-top-30 text-center" >
					<!--<img src="assets/images/logo.png" alt="lancerShelf"/> -->
					<h2 class="text-center text-primary emmabold"> Lancershelf</h2>
				</div>
				<!-- start: LOGIN BOX -->
				<div class="box-login">
					<form class="form-login" id="login" action="#">
						<fieldset>
							<legend>
...
var newDateString = dateObj.getMilliseconds();
        $.ajax({
            url: 'modules/login.php?id='+ newDateString,
            type: 'POST',
            data: formData,
            success: function (data) {
				
				$('#post-response').addClass('alert alert-info');
				$('#post-response').html(data);
			   var res = data.match(/successful/gi);
				
			   if(res.length > 0){ location.href = 'dashboard.php'; }
