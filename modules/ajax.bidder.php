...
$test_query = mysql_query("INSERT INTO `bids`(`project_id`, `bid_price`, `uid`, `completion_time`, `bid_description`, `extra_services`)
		...
		if($test_query){
			...
			$match_query  =	mysql_query("UPDATE `bids` SET `bid_price` = '$bid_price',`completion_time` = '$completion_time',
										`bid_description` = '$bid_description', `extra_services` = '$extra_services'
										WHERE project_id = '".$project_id."' AND uid = '".$uid."' ");
